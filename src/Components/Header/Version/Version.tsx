...
import { v4 } from 'uuid';
import { MenuItems, ModalTextInformation, VersionData } from '../../dataType';
...
const Version: React.FC<VersionProps> = ({ onVersionSelect, selectedCustomer }) => {
  ...
  useEffect(() => {
    const fetchVersions = async () => {
      ...
      if (selectedCustomer) {
        const versions: VersionData[] = data.data[selectedCustomer]?.versions || [];


        const uniqueItems = versions.map((v: VersionData) => ({
          id: v4(),
          item: v.version,
        }));
